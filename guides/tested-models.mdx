...
<Note>
The five tables below are generated from those JSON files by
...
- **A Tesla V100** (Dell C4130) serving through the catalog, solo.
...
- **Two-node TP=2 through the current launch path** (DeepSeek V4 Flash,
  Qwen3.8-Flash-Next), and **four-node TP=4** measured June 2026 on 0.4.x.
- **Correctness separately from speed**: the rubric, the coding harness runs,
  and the agentic rubric.
- **Typed decisions** and **embeddings**, each with its own bench section.
...
- **TP=4 through the current launch path.** The 235B row was measured on 0.4.x
  with the `scitrera/dgx-spark-vllm` image. It has not been re-run on a 0.5.x
  release, and it has no concurrency sweep, no sustained number and no rubric.
- **The C4130's other GPUs.** The V100 row is one GPU of the four in that
  chassis, solo.
...
(the decision bench) and `ainode-bench embed` (the embedding bench). Each has
`--help`. The record format is in
